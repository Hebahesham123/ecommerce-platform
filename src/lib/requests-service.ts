...
/**
 * `viewerPhone` is the shopper this server has already authenticated — a
 * session cookie on the website, a bearer token in the app. It is a parameter
 * rather than something read in here so both surfaces can share the function,
 * which is also exactly why this module is not a Server Action module: an
 * exported action taking a phone would let any browser file requests as
 * anyone.
 */
export async function submitGeneralRequestFor(
  ...
  try {
    ...
    const sessionPhone = viewerPhone;
    ...
    const { error } = await supabase.from("store_requests").insert({
      ...
      name,
      email: email || null,
      // A signed-in shopper's own number wins over whatever the form said, so
      // an enquiry can never be filed against somebody else's phone. Only a
      // guest gets to type one.
      phone: sessionPhone ?? (typedPhone ? normalizePhone(typedPhone) : null),
