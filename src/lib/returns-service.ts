...
/**
 * Returns and exchanges, with the shopper stated rather than assumed.
 *
 * Every function here takes the phone as its first argument, and that argument
 * is the shopper whose data is read and written. It therefore must never be
 * reachable from a browser: a Server Action's arguments are supplied by the
 * caller, so an exported action taking a phone is an exported action that
 * hands anyone anyone else's orders.
 *
 * So this module is not a Server Action module. The website's actions wrap it
 * and pass the session cookie's phone; the mobile API wraps it and passes the
 * bearer token's. Both are phones this server verified for itself.
 */
 ...
 */
export async function listReturnableOrdersFor(
  phone: string | null,
 ...
 */
export async function submitReturnRequestFor(
  phone: string | null,
  ...
  try {
    ...
    const available = await listReturnableOrdersFor(phone);
    ...
    if (kind === "exchange") {
      ...
      for (const want of payload.replacementLines) {
        ...
        if (String(it.status ?? "active") !== "active") {
          return { ok: false, error: "replacement_not_found" };
        }
        // A shopper cannot exchange into stock we do not have. Without this an
        // exchange for ten thousand of something lands on the merchant's desk
        // as a five-figure balance owing, and can never be completed — the
        // stock function will refuse it.
        const levels = Array.isArray(it.inventory_levels) ? (it.inventory_levels as Row[]) : [];
        const onShelf = levels.reduce(
          (sum, l) => sum + Math.max(0, n(l.on_hand) - n(l.committed)),
          0,
        );
...
export async function listMyRequestsFor(
  phone: string | null,
