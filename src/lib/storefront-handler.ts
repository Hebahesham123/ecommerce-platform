...
var pend=false;function scan(){pend=false;var ns=document.querySelectorAll('input[type=number],input[name=quantity],input[name^=updates],input[class*=quantity]');for(var i=0;i<ns.length;i++){if(isQty(ns[i]))guardInput(ns[i]);clamp(ns[i],true);}pdpGuard();}
