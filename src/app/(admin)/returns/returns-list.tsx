...
/** One list, mounted at /returns for everything and /app/returns for the app.
 *  See the note on OrdersList — a forked copy drifts, and two screens that
 *  disagree about the same request are worse than one screen with a filter. */
...
export function ReturnsList({ lockChannel }: { lockChannel?: Channel } = {}) {
    ...
    <>
      <PageHeader
        title={
          lockChannel
            ? `${ar ? "الاسترجاع والاستبدال" : "Returns & exchanges"} — ${
                CHANNEL_LABELS[lockChannel][ar ? "ar" : "en"]
              }`
            : ar
        ...
        subtitle={
          lockChannel === "app"
            ? ar
              ? "طلبات الاسترجاع والاستبدال المفتوحة من التطبيق فقط"
              : "Returns and exchanges opened in the app only"
            : ar
      ...
      <Card className="overflow-hidden">
        ...
        <Toolbar>
          ...
          {(q || status !== "all" || tab !== "all" || (!lockChannel && channel !== "all")) && (
            <button
              onClick={() => {
                setQ("");
                setStatus("all");
